...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if input_ch == ':wq':  # :q 为停止符
            break
        elif input_ch == '':
            pass
        else:
            data_list.append(input_ch)

    d1 = {}
    for a in data_list:
        d1.update({a[:5].strip(): a[5:].strip('\n').strip()})
    ...
    d2 = {}
    for a in data_list:
        d2.update({a[:5].strip(): a[5:].strip('\n').strip()})

    # 开始
    leng = len(d1)
    counter = 1
    while 1:
        for every_d1_record_time in d1:
            for every_d2_record_time in d2:

                floor1 = re.findall('(\d{2}\.\d)/(\d{2}\.\d)', d1[every_d1_record_time])
                wl, zh, room2, room3 = floor1[0], floor1[1], floor1[2:5], floor1[5]

                floor2 = re.findall('(\d{2}\.\d)/(\d{2}\.\d)', d2[every_d2_record_time])
                ...
                # 删除d2已完成的条目
                del d2[every_d2_record_time]
                # print(len(d2))
                break

            # 删除d1已完成的条目
            del d1[every_d1_record_time]
            # print(len(d1))
            break
