...
def qyt_rec_mail(mailserver, mailuser, mailpasswd, mail_number, save_file=False, delete_email=False):
    print('Connecting...')
    ...
    try:
        ...
        # print('There are', msgCount, 'mail messages in', msgBytes, 'bytes')  # 打印邮件数量与字节数
        # print(server.list())  # 打印邮件清单

        # for i in range(msgCount):  # 逐个读取邮件
        hdr, message, octets = server.retr(mail_number + 90)  # 读取邮件
...
if __name__ == '__main__':
    # 使用Linux解释器 & WIN解释器
    mail_number = 1
    for x in qyt_rec_mail('pop.qq.com', '2426848309@qq.com', 'rhrogewmkjtwdifg', mail_number, save_file=False, delete_email=False):
        # print('='*50, '第', i, '封信', '='*50)
