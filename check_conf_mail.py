...
def ssh_multicmd(ip, username, password, cmd_list, verbose=True):
    ...
    for cmd in cmd_list:  # 读取命令
        ...
        chan.send(b'\n')  # 一定要注意输入回车
        time.sleep(10)  # 由于有些回显可能过长，所以可以考虑等待更长一些时间
...
def check_conf(device_list, username, password, cmd_list):
    for sdevice in device_list:
        new, md5_value = get_config_md5(device_list, username, password, cmd_list)

        # 判断文件是否存在 T/F
        if os.path.isfile('check_config.txt'):

            # 若存在 就读取txt中保存的md5值 与当前值进行比较
            with open('check_config.txt') as f:
                # 与之前相同 就略过
                if md5_value == f.read():
                    pass
                ...
                # 与之前不同 发送邮件告知不同 并覆写配置及md5值
                    ...
                    with open('{}_config.txt'.format(sdevice)) as conf:
                        old = conf.read()
                        conf.write(new)
                    differences = diff_txt(old, new)

                    send_mail('smtp.qq.com',  # 邮箱服务器
                              '123456789@qq.com',  # 用户名
                              'dmyymagcazklcjie',  # 密码
                              '123456789@qq.com',  # 发件人邮箱
                              '987654321@qq.com',  # 收件人邮箱
                              '123123123@qq.com;321321321@qytang.com',  # 抄送
                              '设备{0}配置改变'.format(sdevice),  # 标题
...
if __name__ == '__main__':
    ...
    check_conf(['192.168.1.1', '192.168.1.2'], 'root', 'huawei', ['dis cu', ' ', ' '])
