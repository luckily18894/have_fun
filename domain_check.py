...
def output(payload, state):
    ...
    if table:
        ...
        # 客户ip字典
        c_dict = {
            '顺网': ['115.231.111.96/27', '124.90.38.96/27', '61.130.11.192/27', '115.231.111.64/27', '124.90.38.64/27',
                   ...
                   '115.236.57.0/26', '183.131.182.8/29', '121.52.242.96/27', '121.52.247.160/27', '122.224.114.128/26',
                   '122.224.199.224/27', '122.224.178.106', '122.224.178.107', '122.224.178.108', '122.224.178.109',
                   '122.224.178.110', '122.224.178.111', '122.224.178.112', '122.224.178.113', '122.224.178.114',
                   '122.224.178.115', '122.224.178.116', '122.224.178.124', '122.224.178.125'],
        ...
        asdf.to_excel('domain_{}.xlsx'.format(state),  # 文件名
                      ...
                      columns=['所属客户', '解析IP', '域名', '最后访问页', '备案号', '单位', '最后访问时间', '系统ID', '目的IP', '计数']
                      # 列排序（实际作用是指定输出哪几列）
                      )
